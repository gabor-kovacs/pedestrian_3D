...
class TrackingViewer:
    ...
    def generate_view(self, objects, current_camera_pose, tracking_view, tracking_enabled):
        ...
        self.draw_peds(objects, tracking_view, current_camera_pose)
...
#----------------------------------------------------------------------
    ...
    def draw_peds(self, pedestrians, tracking_view, current_camera_pose):
        for pedestrian in pedestrians.object_list:
            if not np.isfinite(pedestrian.position[0]):
                continue
            clr = generate_color_id_u(pedestrian.id)
            ...
            gaze_scale = 2.0
            cv_gaze = self.to_cv_point([pt.get_xyz()[0] + gaze_scale * pedestrian.gaze_vector[0], 
                                        pt.get_xyz()[1] + gaze_scale * pedestrian.gaze_vector[1], 
                                        pt.get_xyz()[2] + gaze_scale * pedestrian.gaze_vector[2]], 
                                        current_camera_pose)
            cv_gaze_point = (int(cv_gaze[0]), int(cv_gaze[1]))
            cv2.circle(tracking_view, cv_point, 4, clr, 4)

            black = (0, 0, 0, 255)

            if pedestrian.angle < 45:
                gaze_clr = (0, 255, 0, 255)
            elif pedestrian.angle < 90:
                gaze_clr = (0, 128, 255, 255)
            else:
                gaze_clr = (0, 0, 255, 255)

            cv2.arrowedLine(tracking_view, cv_point, cv_gaze_point, gaze_clr, 5,)


            text = str(pedestrian.id)
            # cv2.putText(tracking_view, text, (int(cv_pos[0]) + 0, int(cv_pos[1])), cv2.FONT_HERSHEY_SIMPLEX, 1.0, black)
